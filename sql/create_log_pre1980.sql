...
CREATE TABLE logs.log_before1980 (
...
CREATE TRIGGER movie_insert 
    AFTER INSERT 
    ON node_before1980.node FOR EACH ROW 
BEGIN
 INSERT INTO logs.log_before1980(transaction_date, action, row_id, name, year, rating, genre) 
 VALUES (NOW(), "INSERT", NEW.id, NEW.name, NEW.year, NEW.rating, NEW.genre);
END$$
DELIMITER ;

CREATE TRIGGER movie_update 
    AFTER UPDATE ON node_before1980.node
    FOR EACH ROW 
